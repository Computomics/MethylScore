...
if ($OPT_verbose) {
  ...
  foreach my $varname (sort keys %CFG::) {
    print STDERR "$varname: " . ${$CFG::{$varname}} . "\n";
  }
  print STDERR "#######################\n\n";
}
...
$SLEEP_TOTAL += $iter;
            ...
            for my $mr_batch (@MR_batches) {
                if (! -e "$mr_batch.out/done" || $CFG::FORCE_RERUN) {
                    unlink("$mr_batch.out/done") if (-e "$mr_batch.out/done" && $CFG::FORCE_RERUN);

                    # check executable first:
                    abort_pipeline("Cannot find $CFG::BIN_PATH/dmrs!\n") if (! -e "$CFG::BIN_PATH/dmrs");
                    # check matrix file:
                    if (! -e $matrixfile) {
                      if (-e "$matrixfile.gz") {
                        $matrixfile .= ".gz";
                    ...
                    my $cmd = "$CFG::BIN_PATH/dmrs " .
                              "-s $samplefile " .
                              "-r $mr_batch " .
                              "-m $matrixfile " .
                              "-p " . $CFG::MR_FREQ_CHANGE . " " .
                              "-i " . $CFG::CLUSTER_MIN_METH_DIFF . " " .
                    ...
                    my $call_cmd = "$CFG::SCRIPT_PATH/call_DMRs.sh $stage \"$cmd\" $mr_batch.out";
            ...
            print STDERR "[MethylScore] " . localtime() . " - ... Merge DMRs\n" if ($OPT_verbose);

            my $DMRfiles = ();
            for (my $batch_nr=0; $batch_nr<scalar(@MR_batches); ++$batch_nr) {
              my $batch_file = "$DMR_FOLDER/batches/MRbatch.$batch_nr.out/segments.dif";
              $DMRfiles .= " " . $batch_file if (-e $batch_file)
            }
            abort_pipeline("Cannot find DMR files to merge!\n") if ($DMRfiles eq "");

            my $cmd = "cat $DMRfiles > $DMR_FOLDER/segments.dif";
            my $ret = system($cmd);
            abort_pipeline("DMR calling (catting segments.dif's) failed\n") if ($ret);

            abort_pipeline("Cannot find $CFG::BIN_PATH/merge_DMRs\n") if (! -e "$CFG::BIN_PATH/merge_DMRs");
            my $cmd = "$CFG::BIN_PATH/merge_DMRs " .
                      "$samplefile " .
                      "$DMR_FOLDER/segments.dif " .
                      "$DMR_FOLDER " .
                      "$CFG::PYTHON_PATH " .
                      "$CFG::SCRIPT_PATH/pv2qv.py " .
                      "$CFG::FDR_CUTOFF " .
                      "$CFG::CLUSTER_MIN_METH " .
                      "$CFG::DMR_MIN_C " .
                      "$CFG::HDMR_FOLD_CHANGE " .
                      "$CFG::REMOVE_INTMED_FILES " .
            ...
            if ($ret == 0) { # on success:
              ...
              abort_pipeline("DMR calling (merging step) failed\n");
            }
         }
