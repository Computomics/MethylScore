...
include { SAMPLESHEET } from '../get_sheets'
include { MRS         } from '../get_MRs'
include { DMRS        } from '../get_DMRs'

workflow METHYLSCORE {
    take:
    samples
    fasta
    ...
    CONVERT(
        samples.groupTuple(by: 1).combine(fasta),
        "MethylScore"
    )

    MATRIX(
        CONVERT.out.converted.map{ chromID, group, sampleID, context, bedGraph -> [chromID, sampleID, bedGraph] },
        samples.map { group, sampleID, context, path -> sampleID }.unique()
    )

    SAMPLESHEET(MATRIX.out.matrixWG)
