...
### Initialize LOG file:
    ...
    print STDERR "[DMRcalling] ".localtime()." - Starting $context context\n" if ($verbose);
...
### Prepare matrix file, jump to first non-header line:
    my $Mfile = Compress::BGZF::Reader->new_filehandle($matrixfile) or abort("\n{DMRcalling} ERROR: Cannot open matrix file $matrixfile!\n");
    my %M=();
    my $mline = "#";
    my @m;
    while ($mline =~ m/^#/ || ($m[2] ne $context && $context ne "combined")) {
      $mline = <$Mfile>;
      chomp $mline;
      @m = split " ", $mline;
    }
...
#    # jump to the first position of the current chromosome $chr in %M
#    while (m[0] ne $chr || m[2] ne $context) {
#        $mline = <$Mfile>;
...
#    }
    ...
    # parse MR frequency along genome based on MR file:
    my %MRfreqs=( 0 => 0, );
    my $MRfreq = 0;
    my @positions=();
...
print "start of MR block $pos\n" if ($DEBUG);
            ...
            # reset 'visited_position' dict %M:
            %M=() if ($m[1] < $start_of_MRblock);
...
print "pos $pos / freq $MRfreq compared to $cmp_pos / ".$MRfreqs{$cmp_pos}."\n"  if ($DEBUG);
                ...
                if ( ($cmp_pos == -1 && scalar(@segments) > 0) ||
...
print "overwrite previous segment end from " . $segments[-1]{"end"} if ($DEBUG);
                            $segments[-1]{"end"} = $pos;
print " to $pos\n" if ($DEBUG);
...
print "new segment start at $adj_start\n" if ($DEBUG);
                      }
                    }
...
#TODO include flag if one wants to test for variability even if there are no clusters found! (i.e. if K=1)
    ...
    if ($segm{"K"} > 1 && max(@{$segm{"cluster_means"}}) >= $OPT_min_cluster_mean && $DO_TESTING) {
        ...
        my $do_grep = "";
        $do_grep = " | grep \"$context\" " if ($context ne "combined");
        my $return = system(
...
print "   M:" . join(" ", sort{$a<=>$b} keys %M) . "\n" if ($DEBUG);

    if (!exists $M{$pos}) {
        # $M{$pos} does not exist. This can have two reasons:
...
print "(2) m0: " . $m[0] . " m1: " . $m[1] . " $m[2] pos: $chr : $pos\n" if ($DEBUG);
            ...
            while ($m[2] ne $context && $context ne "combined") {
                ...
                chomp $mline;
                @m = split/\t/, $mline;
                last if ($m[0] eq "" || $m[0] ne $chr);
            }

print "(3) m0: " . $m[0] . " m1: " . $m[1] . " $m[2] pos: $chr : $pos\n" if ($DEBUG);
            # store the next pos (in the correct context) in %M, even if it's outside of segment
            # (store not if on diff. chr., since then %M is reset anyways)
            if ($chr eq $m[0]) {
                $M{$m[1]}{"exists"} = 1;
                if ($prev_mchr eq $chr) {
                    $M{$prev_mpos}{"next"} = $m[1];
                }
            }
print "   M:" . join(" ", sort {$a <=> $b} keys %M) . "\n" if ($DEBUG);

            # Finally check if we're in, or if we iterated too far (i.e. there are no positions in this context)
            if ($chr eq $m[0] && $m[1] >= $pos && $m[1] <= $end) {
                $pos = $m[1];
            }
            else {
print "   -> pass 0 (no pos in context)\n" if ($DEBUG);
                return(0, \%sites_per_sample, \%rates_per_sample);
            }

        }
        elsif ($chr eq $m[0] && $m[1] >= $pos) {
            # We have been in segment before -> iterate to next $M entry
            while (!exists $M{$pos} && $pos < $end) {
                $pos++;
print "m1/pos: $m[1]/$pos\n" if ($DEBUG);
            }
        }
        else {
            abort("Error in segment $chr: $start - $end, pos $pos, m: $m[0] : $m[1], $m[2]");
        }
    }


print "  GET_COVERAGES, start-end $start-$end pos $pos m1 ".$m[1]." m2 $m[2]\n" if ($DEBUG);

    # It can be that even now no position exists in the focal context, but then: $pos==$end
    ...
    while ($pos < $end) {
        ...
        if (!exists $M{$pos}{"next"}) {
            ...
            do {
                ...
                chomp $mline;
                @m = split/\t/, $mline;
                $break_iter = ($mline eq "" || $m[0] ne $chr);
            } while (!$break_iter && ($m[2] ne $context && $context ne "combined"));

            last if ($break_iter);
            $M{$pos}{"next"} = $m[1];
            $M{$m[1]}{"exists"} = 1;
            $pos = $m[1];
print "  new m1:  ".$m[1]." m2 $m[2]\n" if ($DEBUG);
        }
        else {
            $pos = $M{$pos}{"next"};
print "  new pos (already existant in %M): $pos\n" if ($DEBUG);
        }
    }
...
print "  subsegment covs: " . Dumper($sites_per_sample) if ($DEBUG);
        ...
        if ($pass) {
        ...
        elsif (scalar(keys %$sites_per_sample) > 0) {
            ...
            if ($max2nd_cov_sites_per_samples == 0 ||
              ...
              $start = $end;
              $end = ($end + $window_size) > $global_end ? $global_end : ($end + $window_size);
              $adj_start = $start;
...
print "update previous subsegment\n" if ($DEBUG);
        ...
        $end = ($end + $window_step > $global_end) ? $global_end : $end + $window_step;
    }
...
$mid_time = time() - $start_time;
    ...
    for (; $K <= int(100/($OPT_min_cluster_diff+0.1)); ++$K) {
...
-c   STRING         Sequence context to analyze (CG, CHG or CHH, or combined). Sample grouping
                    and coordinates will be different between context-specific DMRs.
...
\n");
	...
	GetOptions(\%CMD, "s=s", "m=s", "o=s", "c=s", "v=i", "n=i", "a=i", "p=f", "d=i", "w=i", "x=i", "z=i", "i=f", "j=i", "f=f", "B=s", "T=s", "E=s", "Y=s", "K=s", "l", "r=s", "q", "u=i", "g", "no-post-process");

	die("Please specify strain file (-s)!\n[ABORTED]\n") unless defined($CMD{s});
	die("Please specify matrix file (-m)!\n[ABORTED]\n") unless defined($CMD{m});
	die("Please specify output folder (-o)!\n[ABORTED]\n") unless defined($CMD{o});
  die("Please specify sequence context (-c)!\n[ABORTED]\n") unless defined($CMD{c});
  ...
  $OPT_window_step = $OPT_window_size if ($OPT_window_size > 0 && $OPT_window_step == 0);
  if ($OPT_window_size>0 && $OPT_window_step>0 && $OPT_window_step > $OPT_window_size) {
  ...
  if (defined($CMD{q})) { $verbose = 0; $DEBUG = 0; }
  if (defined($CMD{g})) { $DEBUG = 1; }
